...
void ReqURL(int i) {
  ...
  if (https.begin(client, URL[i])) {  // Set HTTPS request for URL i
    ...
    if (httpCode > 0) {
      ...
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
        ...
        Serial.println("GETTING URL RESPONSE FROM URL ROUTINE SERVER");
        Serial.println(payload);      // Show web server response       
      }

    // https.begin(client,WURL[i]);
    // httpCode = https.GET()
    } else {
  ...
  if (https.begin(client, WURL[i])) {  // Set HTTPS request for URL i
...
void strangerDetected(){
  ...
  if (https.begin(client, strangerURL)) {  // Set HTTPS request for URL i
    ...
    if (https.begin(client, strangerWURL)) {  // Set HTTPS request for URL i
